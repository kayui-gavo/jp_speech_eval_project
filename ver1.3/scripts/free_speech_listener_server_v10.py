...
$("audio").src=`/api/audio/${encodeURIComponent(current.audio_asset_id)}`;$("audio").load();$("questions").innerHTML=current.assigned_constructs.map(ratingQuestion).join("");clearForm()}
